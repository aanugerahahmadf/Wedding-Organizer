<div class="overflow-x-hidden">
    ...
    @include('filament.user.partials.legal-header', ['active' => 'privacy'])
    ...
    <div x-data="{ active: '' }"
            ...
            <nav class="space-y-0.5">
                @foreach ($sections as $i => $section)
                    <button
                        ...
                        <span class="uppercase tracking-wide truncate">{{ $section['heading'] }}</span>
                    </button>
                @endforeach
            </nav>
        ...
        <div class="w-full bg-white dark:bg-gray-950">
            <div class="max-w-7xl mx-auto px-6 lg:px-16 py-12 lg:py-20 mb-60">
                ...
                <header class="space-y-0 pb-2 border-none">
                    <h1
                        ...
                        {{ __($pageRecord?->title ?? 'Privacy Policy') }}
                    </h1>

                    <p class="text-sm leading-relaxed text-slate-600 dark:text-slate-400 text-justify m-0">
                        {{ __('Kami menjaga privasi Anda dengan serius. Dokumen ini menjelaskan bagaimana data Anda dikumpulkan, digunakan, dan dilindungi.') }}
                    </p>
                </header>
                ...
                <div class="flex justify-center pb-60">
                    <x-filament::button tag="a" href="{{ route('filament.user.auth.login') }}"
                        icon="heroicon-m-check-circle" icon-position="after" size="xl" color="success" class="shadow-xl scale-110 rounded-xl">
                        {{ __('Saya Setuju & Masuk') }}
                    </x-filament::button>
                </div>
            </div>
        </div>
